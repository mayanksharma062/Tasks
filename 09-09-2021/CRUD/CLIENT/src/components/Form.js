...
export default function Form(){
    ...
    const addData = ()=>{
        Axios.post('http://localhost:3050/create', {name:name, age:age, phone:phone}).then(()=>{
          showdata([...show, {name:name, age:age, phone:phone}])
        })
    }

    const handleinfo = ()=>{
      Axios.get('http://localhost:3050/show').then((response)=>{
    ...
    const handleupdatephone = (id)=>{
      // alert(id);
      Axios.put('http://localhost:3050/update', {phone: updatePhone, id:id}).then((response)=>{
...
<div>
  ...
  <button type="button" onClick={handleinfo} className="btn my-3 btn-lg btn-success">Show Data</button>
  {show.map((val)=>{
    return (
      <>
      <h3  className="text-danger">New Data</h3>
